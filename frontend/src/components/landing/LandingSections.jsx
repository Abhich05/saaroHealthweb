...
// Fixed Saaro Landing Sections - all JSX syntax errors resolved

export const Hero = () => (
  <section className="w-full bg-gradient-to-br from-gray-50 to-purple-50 py-20 px-6 text-center">
    <h1 className="text-4xl lg:text-6xl font-extrabold text-gray-800 mb-4 leading-tight">
      AI-Powered
    </h1>
    <p className="text-2xl lg:text-4xl font-semibold text-purple-700">
...
export const TrustSection = () => (
  <section className="py-12 bg-white text-center">
    ...
    <div className="flex flex-wrap items-center justify-center gap-8 opacity-75" role="list">
      {Array.from({ length: 5 }).map((_, i) => (
        <div
          key={i}
          className="w-32 h-8 bg-gray-200 rounded"
          aria-label={`Placeholder logo ${i + 1}`}
        />
      ))}
    </div>
  </section>
    ...
    {featureData.map(({ heading, sub, bullets }, idx) => (
      ...
      >
        <div className="max-w-5xl mx-auto px-6 flex flex-col md:flex-row items-center gap-10">
          ...
          <div
            className="flex-1 h-64 bg-gray-200 rounded-lg"
            aria-label={`Feature image placeholder for "${heading}"`}
          />
        </div>
      </section>
    ))}
          ...
          <div key={i} className="bg-gray-50 p-6 rounded-lg shadow text-sm">
            <div className="flex items-center mb-3">
              <div
                ...
                aria-label={`Doctor avatar placeholder ${i + 1}`}
              />
              <div>
            ...
            <p className="italic">
              "Saaro's AI tools have transformed my daily workflow and improved patient outcomes."
            </p>
          </div>
        ))}
      </div>
...
export const BottomCTA = () => (
  <section className="py-20 bg-purple-700 text-center text-white">
    ...
    <button
      className="bg-white text-purple-700 font-semibold px-6 py-3 rounded-full shadow hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-purple-300 transition"
      aria-label="Sign Up for Saaro"
    >
      Sign Up Free
    </button>
  </section>
